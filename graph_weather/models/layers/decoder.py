...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        for node_index, _ in enumerate(self.h3_grid):
            # Get h3 index
    ...
    def forward(self, processor_features: torch.Tensor, start_features: torch.Tensor) -> torch.Tensor:
        ...
        edge_attr = einops.repeat(edge_attr, "e f -> (repeat e) f", repeat=bs)

        edge_index = torch.cat(
            [self.graph.edge_index + i * torch.max(self.graph.edge_index) + i for i in range(bs)],
            dim=1,
        )

        # Read nodes to match graph node number
        self.latlon_nodes = self.latlon_nodes.to(processor_features.device)
        features = einops.rearrange(processor_features, "(b n) f -> b n f", b=bs)
        features = torch.cat([features, einops.repeat(self.latlon_nodes, "n f -> b n f", b=bs)], dim=1)
